...
2. 按"Chapter X"或"chapter x"切分文档，统一输出为"Chapter X"格式
...
DEFAULT_INPUT_FILE = 'txt/DavidCopperfield.txt'  # 默认输入文件
...
def split_into_chapters(text):
    ...
    for line in lines:
        ...
        if re.match(chapter_pattern, line.strip()):
            ...
            # 开始新章节，统一转换为大写格式
            current_chapter = line.strip().replace('chapter', 'Chapter')
            current_content = []
        else:
            # 添加到当前章节内容
            if current_chapter is not None:
